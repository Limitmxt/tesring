...
This app is a **PWA (Progressive Web App)** — it installs to your home screen
and runs full-screen like a normal app, no app store required. You just need the
...
The icon is generated by `scripts/generate-icons.mjs` (run `node
scripts/generate-icons.mjs` to regenerate). A basic service worker
(`public/sw.js`) provides the installability hook and a simple offline cache.

> Note: live eBay search and the AI classifier still need network + the API
> keys configured on the server. Offline mode just keeps the app shell and
